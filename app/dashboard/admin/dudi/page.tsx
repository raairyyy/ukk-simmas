// FILE: app/dashboard/admin/dudi/page.tsx
"use client"
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-900">
      ...
      <aside className="w-[280px] bg-white border-r border-slate-100 hidden md:flex flex-col fixed h-full z-20 px-6 py-8">
        <div className="flex items-center gap-4 px-2 mb-10">
          {/* FIX 1: Logo Profil Jadi KOTAK (rounded-xl), bukan rounded-full */}
        ...
        <nav className="flex-1 space-y-4">
          <SidebarItem 
            ...
            label="Dashboard" 
            subLabel="Ringkasan sistem" 
            href="/dashboard/admin" 
          />
          <SidebarItem 
            icon={<Building2 size={24} />} 
            label="DUDI" 
            subLabel="Manajemen DUDI" 
            active 
          />
          <SidebarItem 
            icon={<Users size={24} />} 
            label="Pengguna" 
            subLabel="Manajemen user" 
          />
          <SidebarItem 
...
function SidebarItem({ icon, label, subLabel, active, href }: any) {
  return (
    ...
    >
      ...
      <div>
        <p className={`text-base ${active ? "font-bold" : "font-semibold"}`}>{label}</p>
        <p className={`text-xs mt-0.5 ${active ? "text-blue-100 font-medium" : "text-slate-400"}`}>{subLabel}</p>
      </div>
    </div>
  )
}
...
function DudiRow({ name, address, email, phone, pic, status, count }: any) {
  ...
  return (
    <TableRow className="group hover:bg-slate-50/80 border-slate-100 transition-colors">
      ...
      <TableCell className="text-center">
         {/* FIX 2: Badge Siswa Magang Jadi KOTAK (rounded-lg) */}
         {/* Warna: Olive/Emas Gelap (#84cc16) sesuai desain asli atau referensi */}
